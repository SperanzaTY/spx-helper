...
<!-- 本 Skill 位于 .cursor/skills/，版本随 SPX_Helper 仓库维护。
     维护：一律手动编辑与手动 cp 同步（勿依赖 sync_skills.sh）；全局↔工程方向见 docs/SKILL_INSTALL.md。
     全局→工程时若仓库已有他人提交的 Skill，须手动 diff/合并，勿整目录覆盖。
     版本演进应在编辑器内 diff/合并，由人与 Agent 按语义优化，避免盲覆盖。
...
```sql
-- 探查列名与样本（Presto 上 grass_date 多为 varchar，用字符串 'YYYY-MM-DD'，勿用 DATE '...' 以免类型不匹配）
SELECT *
...
**更新方式**：在编辑器中直接修改 `.cursor/skills/spx-bug-trace/SKILL.md`（及关联 md），在对应位置追加或修改；更新后在 `## Skill 更新记录` 节记录本次改动。需要与 `~/.cursor/skills/spx-bug-trace` 对齐时，**手动** `cp` 或 diff 合并，**不使用** `sync_skills.sh` 作日常同步（见 `docs/SKILL_INSTALL.md` 维护原则）。
...
| 2026-03 | **维护约定**：Skill 一律手动编辑与手动 cp 同步，不依赖 `sync_skills.sh`；`docs/SKILL_INSTALL.md`、`.cursorrules`、Phase 7、`scripts/sync_skills.sh` 头注释同步 | 团队约定：用理解与合并优化版本，避免脚本盲覆盖 |
| 2026-03 | **协作**：全局→工程时若工程侧已有他人提交的 Skill，须手动 diff/合并，禁止整目录覆盖；见 `docs/SKILL_INSTALL.md` | 团队补充协作流程 |
