...
| `get_flink_app_detail` / `get_flink_instance` (flink-query MCP) | 获取 Flink 应用详情、实例状态（当前/历史） |
| `get_flink_exceptions` / `get_flink_runtime_exceptions` (flink-query MCP) | 获取 Flink 异常列表和完整堆栈 |
| `get_flink_operation_log` (flink-query MCP) | 获取 Flink 操作日志（启动/停止/重启记录） |
...
**[推荐] 使用 Flink MCP 工具链快速定位任务**：

当确认 CK 表断流后，用 Flink MCP 工具自动化定位问题，而非手动去 DataSuite 页面排查：

**Step 1：从表名反推任务名，搜索 Flink 任务**

```
search_flink_apps(keyword="<snake_case 核心表名片段>", project_name="spx_mart")
```

注意 DataSuite 搜索 API 的模糊匹配**非常不精确**，经常返回无关结果。应对策略：
1. 用尽可能短且唯一的关键词（如 `volume10m_tab` 而非完整任务名）
2. 若搜索不到，用 **SeaTalk 告警群消息** 反查 appId（见下方）
3. 从已知的同系列任务（如 SG/VN）的 appId 推测相邻 appId

**Step 2：SeaTalk 告警群搜索（搜索 API 不可靠时的替代方案）**

当 DataSuite 搜索找不到目标任务时，用 SeaTalk MCP 搜索告警群消息：
...
get_flink_exceptions(app_id=<appId>)        # 异常列表
get_flink_operation_log(app_id=<appId>)     # 操作日志
get_flink_metrics(app_id=<appId>, category="kafka")  # Kafka Lag
get_flink_checkpoints(app_id=<appId>)       # Checkpoint 状态
```

**Step 4：CK system.parts 验证数据是否曾被写入**

当无法确认 Flink 任务是"从未写入数据"还是"数据被 TTL 删除"时，查询 CK 的 parts 表：

```sql
-- 检查表是否有任何 parts（包括已过期的）
SELECT partition, name, rows, modification_time
FROM system.parts
WHERE database = '<db>' AND table = '<local_table_name>'
ORDER BY modification_time DESC LIMIT 10
```

- 0 行结果 = 数据从未被写入过（不是 TTL 删除）
- 有 parts 但 rows=0 或 modification_time 很旧 = 数据曾写入但已过期/被合并

**Step 5：横向对比健康市场**

对比健康市场（如 SG）和异常市场（如 ID）的 parts 写入状态，判断问题范围：

```sql
-- SG 健康市场：应该能看到持续写入的 parts
SELECT count() as parts_cnt, max(modification_time) as latest_part
FROM system.parts
WHERE database = '<db>' AND table = '<sg_local_table>'
...
#### Layer 6 详细排查路径（单号级排查 — 订单为何缺失）

当用户反馈某个特定订单在看板视图中找不到时（如 Backlog 列表中缺少某单号），按以下步骤排查：

1. **确认视图的筛选条件**：从 biz_sql 提取视图的过滤逻辑（如 Backlog 要求 `first_inbound_aging_time_h >= 24`）
2. **在视图源表中查该单号**：若确认不存在，说明订单不满足视图条件
3. **在 FO RI 表中查该单号的完整生命周期**：`dwd_spx_fo_order_ri_{region}_all`（ck2, `spx_mart_manage_app`），获取 `tracking_status`、各时间戳字段（`soc_inbound_time_map`、`fulfilled_time` 等）
4. **计算订单是否满足视图条件**：如 SOC 停留时间是否 >= 24 小时
5. **得出结论**：订单数据正常但不满足条件 / 订单数据确实异常

**关键注意事项**：
- FO RI 表字段在不同市场可能有差异，务必先 `SELECT * LIMIT 1` 确认字段名
- BR 市场的 FO RI 表没有 `status` 和 `mtime` 字段，应使用 `tracking_status` 和 `_version`
- 某些视图源表是**快照表**（如 `dwd_spx_mm_backlog_order_10m`），只保留最新 `data_timestamp` 的数据，无法回溯该订单是否曾经在视图中出现过
- FO RI 表中 `soc_inbound_time_map`、`hub_inbound_time_map` 等 Map 字段记录了各站点的时间戳，是还原订单完整轨迹的最佳数据源

**SQL 模板**：
```sql
-- 步骤 1：检查订单是否在视图源表中
SELECT * FROM spx_mart_manage_app.<视图源表>_{region}_all FINAL
WHERE shipment_id = '<订单号>'

-- 步骤 2：查 FO RI 表获取完整生命周期
SELECT * FROM spx_mart_manage_app.dwd_spx_fo_order_ri_{region}_all FINAL
WHERE shipment_id = '<订单号>' LIMIT 1

-- 步骤 3：时间戳转换为当地时区（BR 示例）
SELECT toDateTime(<timestamp>, 'America/Bahia') AS local_time
```
