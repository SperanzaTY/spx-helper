...
- **可选校验、仅供参考**：不替代 `get_api_lineage` / 直查库表；与平台或代码不一致时以线上可复现证据为准。
- **更新与沉淀为人工查询追溯**：表数据依赖人工维护或与 Flink 平台同步，可能滞后；排查须与代码、CK/Presto 等探查交叉验证。
- **辅助记录与坑点文档**：app问题整理、坑点用于事后沉淀与排雷，不单独作为「当前根因」的判定依据。

**与排查主路径的关系**：接口侧已定位到 **问题表** 后，典型闭环为 **代码仓库（Metadata/conf）→ Hive `data_metamart.ods_flink_platform_connector_metadata_df`（`query_presto`，平台 connector 事实）→ 可选与本表「任务上下游信息梳理」交叉验证 → 再回到代码 → 迭代 + 数据探查**。GSheet 不是平台元数据的唯一来源；详见 `SKILL.md` **Flink 平台血缘（Hive ODS）** 与 **Phase 2.1b**。
...
| 上下游校验 | 同上 | **任务上下游信息梳理** | Flink 平台任务血缘的人工维护/同步视图（与 SKILL Phase 2.1b、2.2 配合） |
...
**与代码排查的配合**（非「仅对账」）：定位到问题表后，先用仓库理清候选任务与写入链，再用本表对齐 **平台侧** 上下游，再回仓库追上游任务；见 `SKILL.md` Phase 2.1b。
