...
```bash
# 推荐：使用仓库统一 MCP venv，一次性安装所有本地 MCP 依赖
...
- 补充 hatchling wheel 文件选择配置，修复全量 editable install 时无法判断 datamap-query 打包文件的问题。
- 本地安装方式同步为仓库统一 MCP venv：`bash scripts/setup-mcp-env.sh` + `scripts/codex-mcp-launch.sh datamap-query`。
...
- Cookie 过期或收到 401 时会自动尝试 SSO 静默刷新（v3.5.9+；401 时跳过 60s 冷却）；仍失败时错误中含 CDP 与静默刷新诊断（v3.5.17+，`format_auth_troubleshoot`），大多数情况下无需手动干预
