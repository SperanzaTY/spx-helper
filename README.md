...
| **Codex 配置**    | 项目级 `AGENTS.md`、MCP 和 repo-scoped skills，便于从 Cursor 迁移到 Codex                | [CODEX.md](docs/guides/CODEX.md) |
...
项目最初面向 Cursor 构建，当前已补充 Codex 的项目级配置。

#### 方案 A：Cursor IDE（原有流程）

MCP 工具默认可以配合 [Cursor](https://www.cursor.com/) IDE 使用；SeaTalk Agent 在保留 Cursor ACP 路径的同时，也已支持通过 Codex CLI 的 `app-server` 模式接入 Codex。

1. 打开 https://www.cursor.com/ → 点击 **Download**
2. 将下载的 `.dmg` 拖入 Applications
3. 启动 Cursor，使用 GitHub / Google 账号登录

#### 方案 B：Codex（迁移中，已支持项目级 MCP/Skill + SeaTalk Agent app-server）

如果使用 Codex，请参考 [Codex 使用指南](docs/guides/CODEX.md)。
当前 `seatalk-agent` 在 Codex 后端下默认使用 `Approval=never`、`Sandbox=danger-full-access`、`Web Search=live`。
