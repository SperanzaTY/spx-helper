...
[![Version](https://img.shields.io/badge/version-2.19.0-blue.svg)](https://github.com/SperanzaTY/spx-helper/releases)
...
agent --version      # 需要有输出（见下方安装说明）
```

**安装 `agent` CLI**（SeaTalk Agent 专用，普通 MCP 用户无需安装）：

`agent` 是 Cursor 提供的 ACP 命令行工具，SeaTalk Agent 通过它在 Cursor 外部启动 AI Agent 进程。安装方法：

1. 打开 Cursor → 按 `Cmd + Shift + P` 打开命令面板
2. 输入 `install agent`
3. 选择 **"Install 'agent' command"** 并执行
4. 重启终端后运行 `agent --version` 验证

#### 步骤 1：SeaTalk 开启远程调试

SeaTalk 需要以特殊参数启动，让 Agent 能通过 CDP 连接它：

```bash
# 先完全退出 SeaTalk（Cmd+Q），然后用以下命令启动：
open -a SeaTalk --args --remote-debugging-port=19222
```

验证是否生效：在浏览器访问 http://127.0.0.1:19222/json ，如果看到 JSON 数组说明成功。

**永久配置**（免去每次手动启动）：

```bash
# 修改 SeaTalk 的 Info.plist，添加启动参数
defaults write com.aspect.seatalk NSAppTransportSecurity -dict-add AllowsArbitraryLoads -bool YES
# 或创建一个启动脚本 ~/start-seatalk.sh：
echo '#!/bin/bash
open -a SeaTalk --args --remote-debugging-port=19222' > ~/start-seatalk.sh
chmod +x ~/start-seatalk.sh
```
...
[xx:xx:xx] [agent] ACP agent connected (resumed=false)
