...
[![Version](https://img.shields.io/badge/version-3.6.7-blue.svg)](https://github.com/SperanzaTY/spx-helper/releases)
...
- **v3.6.7**：**scheduler-query** 整理 Mart / SLA 批量运维 DataSuite 接口：**`datasuite_bulk_api`** 路径速查；新增 MCP **`search_scheduler_tasks_fuzzy`**、**`list_task_instances_by_update_time`**、**`get_sla_full_configuration`**、**`query_marker_task_bindings`**、**`list_sla_bindings_for_task_instance`**；`_request` / `_sla_request` 支持 **`x-datasuites-project-code`**；**`get_task_instances`** / **`search_tasks`** 默认带项目头；脚本 **`scripts/refresh_datasuite_sla_sheets.py`**（Cookie 自动拉取、多 SLA 保留最晚时刻、实例 `endRunTime` 取最大）与 **`scripts/_ingest_ds_cookie_to_tmp.py`**。发版说明见 [docs/RELEASE_LOG.md](docs/RELEASE_LOG.md)。
