...
[![Version](https://img.shields.io/badge/version-3.6.15-blue.svg)](https://github.com/SperanzaTY/spx-helper/releases)
...
- **v3.6.14**：**MCP 认证与 USEast 查询修复**：chrome-auth 不再把 SeaTalk `19222` CDP proxy 当成 DataSuite 登录浏览器，401 诊断会标出被跳过的非 Chrome 端口；presto-query 的 `idc` 统一支持 `us` / `USEast` 并提交为 DataSuite API 的 `USEast`。发版说明见 [docs/RELEASE_LOG.md](docs/RELEASE_LOG.md)。
- **v3.6.13**：**SeaTalk Agent MCP 状态面板**：左侧 rail 齿轮新增 MCP 状态入口，展示 MCP server / tool schema 实时状态，支持重新检测、安全 smoke tests 与重载 MCP 会话；状态页改为和诊断信息共用 Agent sidebar overlay。发版说明见 [docs/RELEASE_LOG.md](docs/RELEASE_LOG.md)。
