...
| **flink-query** | DataSuite Flink 流/批任务查询与诊断（状态、延迟、异常、血缘、一键诊断） | 浏览器 Cookie 自动认证（chrome-auth） |
...
`scheduler-query`、`datamap-query`、`flink-query`、`datastudio-mcp` 均使用 **chrome-auth** 共享库自动从 Chrome 浏览器获取认证 Cookie，无需手动配置 Token。`get_auth` 会**合并**磁盘 Cookie 与 CDP 实时 Cookie（并修正对 `.shopee.io` 父域 Cookie 的过滤），避免仅读到部分 Cookie 就提前返回。若仍 401：在较新的 macOS Chrome 上 DataSuite 认证 Cookie 可能**加密落盘**，磁盘读不到 `DATA-SUITE-AUTH-userToken-v4`，必须用 **Chrome 远程调试**由 CDP 取全量 Cookie。请按 `mcp-tools/chrome-auth/README.md` 中「DataSuite / Flink / Scheduler MCP 报 401」执行 `scripts/start_chrome_remote_debug.sh` 与 `scripts/verify_datasuite_auth.py`。

遇到 401/403 错误时，这些 MCP 会返回 Cookie 诊断信息（读到几个 Cookie、缺哪些关键 Cookie），并提示"这通常是 Chrome 登录态问题，不是代码 bug"。
...
| `DATAMAP_OPEN_API_TOKEN` | `Basic xxxxxx` | DataMap 写入操作专用（`update_table_info` / `update_column_info`）。联系 yixin.yang@shopee.com 申请。不配置不影响查询功能 |
...
| **chrome-auth 认证** | 无需配置 | `scheduler-query`、`datamap-query`、`flink-query`、`datastudio-mcp` 自动从 Chrome Cookie 获取认证，只需在 Chrome 中登录 [DataSuite](https://datasuite.shopee.io)。Spark 诊断工具还需登录 [Keyhole](https://keyhole.data-infra.shopee.io) |
