...
**scheduler-query 补充（v3.5.18）**：`get_instance_detail`、`get_presto_query_sql`（传 `task_instance_code`）等会经 **`scheduler_task_code.extract_task_code`** 从实例编码解析 **taskCode**（覆盖 DAY/HOUR/MINUTE/MONTH/WEEK/YEAR、`etl_batch` 等形态）；说明与示例见 `mcp-tools/scheduler-query/README.md`，校验命令见该目录 `tests/test_extract_task_code.py`。
