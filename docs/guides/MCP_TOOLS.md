...
| **scheduler-query** | DataSuite Scheduler 任务查询 + Presto/Spark History + Spark 性能诊断；Mart/SLA 批量接口（`search_scheduler_tasks_fuzzy`、`get_sla_full_configuration`、`query_marker_task_bindings` 等，见 `mcp-tools/scheduler-query/README.md`「DataSuite 批量运维接口」与 `datasuite_bulk_api.py`） | 浏览器 Cookie 自动认证（chrome-auth）                    |
...
**presto-query 补充（v3.6.8）**：`query_presto` 现支持 **`max_wait_seconds`**（总轮询等待时间，默认 **600s**）与 **`request_timeout_seconds`**（单次 HTTP 读超时，默认 **60s**），适合大查询或 DataSuite 网关返回较慢的场景；也可通过环境变量 **`PRESTO_MAX_WAIT_SECONDS`**、**`PRESTO_HTTP_READ_TIMEOUT_SECONDS`**、**`PRESTO_HTTP_CONNECT_TIMEOUT_SECONDS`** 调整默认值。目录内的 **`presto_query_tool.py`** 也同步支持相同超时能力，便于非 Cursor MCP 的 Python / 平台 Agent 复用。

**presto-query 直接脚本补充（v3.6.10）**：非 MCP 场景下可直接使用目录内的 **`query_presto.py`**。release 分支保留脚本可执行权限，拉取后可运行 `./mcp-tools/presto-query/query_presto.py "SELECT 1"`；若通过聊天软件、网盘或压缩包单独分发文件导致权限丢失，可改用 `python3 mcp-tools/presto-query/query_presto.py "SELECT 1"`，或先执行 `chmod +x mcp-tools/presto-query/query_presto.py`。

**presto-query 补充（v3.6.3）**：`query_presto` 现使用 **`idc`** 参数切换集群，支持传 **`sg`** / **`us`** / **`USEast`**，默认 **`sg`**；其中 `us` 会规范化为 DataSuite API 使用的 **`USEast`**。返回结果会显示本次命中的 IDC，并统一附带 **实际执行的 SQL**（成功 / 失败 / 超时 / 请求异常），避免和业务 region 概念混淆。目录内另有 **`presto_query_tool.py`**，导出 **`PrestoQueryTool`** 类，便于在非 Cursor MCP 的 Python / 平台 Agent 中直连同一 DataSuite Personal SQL API（凭证仍由调用方传入，勿入库）。说明见 `mcp-tools/presto-query/README.md`。

**ck-query / spark-query / api-trace 补充（v3.6.3）**：这三类查询/溯源 MCP 的返回也统一补充 **实际执行的 SQL**。其中 `api-trace` 会在 lineage 查询步骤显示实际查血缘表的 SQL，后端失败时也会保留原始 SQL 便于排错。

**scheduler-query 补充（v3.5.21）**：在 v3.5.20 基础上增加 **`fetch_mart_sla_instances_from_shortlink`**（``shp.ee`` + 已登录 Cookie 调 **`GET .../sla/slaInstance/get`** 抽取 **taskInstanceCode**）；**`triage_mart_sla_alert`** 增加 **`sla_shortlink_fetch_instances`**（默认 True，正文无编码时用短链补全后再拉 Scheduler 详情，并返回 **`sla_shortlink_fetch_trace`**）。CDP 辅助：`scripts/probe-datasuite-sla-cdp.js`。单测：`tests/test_mart_sla_instance_api.py`。
...
**datamap-query 补充（v3.6.4）**：除表基础信息、字段详情、表级血缘外，现已补齐 4 个更贴近排障的能力：
- **`get_lineage_tasks`**：基于真实 **`/lineageV2/taskInfo`** 返回 upstream/downstream 任务、owner、project、env、活跃状态、详情/code 链接，适合判断“谁在产出 / 谁在消费 / 最近是否还活跃”
- **`get_column_node_info`**：基于 **`/lineage/getColumnNodeInfo`** 返回字段描述、类型、计算逻辑、L30D query、技术 PIC
- **`get_column_lineage`**：基于 **`/lineageV2/filter/column`** 返回字段级 upstream/downstream 血缘原始结果；某些字段可能真实无列级边，此时 `stream` 为空属正常
- **`get_downstream_applications`**：基于 **`/common/downStreamApplication`** 返回 Data Service / OneBI Dataset / DataGo / Dashboard 下游应用

常见自然语言示例：
- `这个表的 upstream task 是谁？最近 32 天还在跑吗`
- `查一下 staff_id 这个字段的节点信息和查询热度`
- `这个字段有没有字段级血缘`
- `这个表有哪些 Downstream Application，特别是 OneBI dataset`
...
- **自动 SSO 刷新**：过期或 401 时自动续期。默认只走 **CDP**（优先 **hidden 目标** + `Page.navigate`，多端口依次尝试），CDP 失败时返回诊断，不再自动打开 Chrome 窗口。确实需要旧兜底行为时可设置 `CHROME_AUTH_ALLOW_BROWSER_OPEN=1`，允许 macOS `open -g` / AppleScript 打开浏览器。推荐 Chrome 使用 `--remote-debugging-port=9222` 并设置 `CHROME_CDP_PORT=9222`，静默程度最高
...
遇到 401/403 错误时，这些 MCP 会返回 Cookie 诊断信息并自动尝试 CDP 静默刷新。仅当 SSO 主会话也过期（需要重新输入密码）时才需要用户手动在 Chrome 中登录 DataSuite。
...
3. 将你要让 MCP 读写的 **Google Spreadsheet** 或其所在 **Google Drive 文件夹** 共享给下面这个 service account：

```text
cursor@spx-helper.iam.gserviceaccount.com
```

4. 权限至少给 **Editor / 可编辑**。如果只给 Viewer，MCP 只能读不能写；如果目标表在某个固定文件夹下，优先直接共享文件夹，避免每个新 Sheet 都要单独授权。
5. 配置：
...
- `SERVICE_ACCOUNT_PATH` 指向上面的 service account 凭据文件，不是你个人 Google 账号的 OAuth token。
- 如果 MCP 提示找不到表、无权限、或只能列出一部分文件，先检查目标 Spreadsheet / 文件夹是否已经共享给 `cursor@spx-helper.iam.gserviceaccount.com`。
- `DRIVE_FOLDER_ID` 建议填团队默认落盘目录；如果你希望 AI 默认在别的文件夹里创建表，改成对应文件夹的 ID 即可。
...
DataSuite 相关 MCP 报 401？

- `chrome-auth` 会跳过 SeaTalk Agent 的 `19222` CDP proxy，不再把它当成 DataSuite 登录浏览器。
- 若诊断里只有 `19222=SeaTalk/CDP-Proxy` 可达，请启动带 remote debugging 的 Chrome，并设置 `CHROME_CDP_PORT=9222`。
- 在同一个 Chrome 中打开并登录 `https://datasuite.shopee.io/studio` 后，重启或刷新 MCP。
...
- **chrome-auth**：DataSuite / Keyhole / Grafana 认证域会跳过 SeaTalk / Electron CDP 代理端口，避免把 `19222=SeaTalk/CDP-Proxy` 当成 DataSuite 登录浏览器；401 诊断会显示 CDP 产品名和跳过原因。
- **presto-query**：`idc` 参数统一支持 `sg` / `us` / `USEast`；`us` 会提交为 DataSuite Personal SQL API 使用的 `USEast`，与元数据工具口径一致。
...
- **presto-query**：新增 `search_presto_tables`、`get_presto_table_metadata`、`preview_presto_table_data`、`get_presto_column_distinct_values`、`review_presto_sql`、`explain_presto_sql` 与 `execute_datastudio_adhoc_query`，让 Agent 不再用 `DESCRIBE` / `SHOW` / `information_schema` 猜元数据。
- **datastudio-mcp**：新增 `search_datastudio_assets` 与 `read_datastudio_asset`，可按项目搜索资产、按 `assetId` 读取 SQL / 脚本内容；Cookie 缓存默认写入 `~/.spx_helper/datastudio-mcp/.config`，避免只读 cwd 失败。
- **chrome-auth**：CDP 静默刷新失败时默认返回诊断，不再自动打开 Chrome；如需旧兜底行为，显式设置 `CHROME_AUTH_ALLOW_BROWSER_OPEN=1`。
...
- **presto-query**：`query_presto.py` 保留可执行权限，支持 Git 拉取 release 后直接执行；同时补充单文件分发时权限丢失的 fallback 调用方式。

### v3.6.4

- **DataMap MCP**：新增 `get_lineage_tasks`、`get_column_node_info`、`get_column_lineage`、`get_downstream_applications`；真实接入 `/lineageV2/taskInfo`、`/lineage/getColumnNodeInfo`、`/lineageV2/filter/column`、`/common/downStreamApplication`
- **DataMap MCP**：`get_table_lineage` 改为使用实际解析后的 qualifiedName，并保留更多实体/任务字段，减少 IDC fallback 后 `current` 对不上的问题
- **文档**：`MCP_TOOLS.md` 与 `mcp-tools/datamap-query/README.md` 同步新增能力与使用场景

### v3.5.20

- **scheduler-query**：**`parse_mart_sla_alert`**、**`triage_mart_sla_alert`**（**`sla_shortlink_fetch_instances`** 短链补全 **taskInstanceCode**）；**`fetch_mart_sla_instances_from_shortlink`**；**`resolve_mart_sla_shortlink`**（``shp.ee`` 还原 DataSuite SLA 详情 URL）；**`extract_task_code`** 增加 **datahub.bti** 形态。
