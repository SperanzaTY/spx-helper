...
遇到 401/403 错误时，这三个 MCP 会返回 Cookie 诊断信息（读到几个 Cookie、缺哪些关键 Cookie），并提示"这通常是 Chrome 登录态问题，不是代码 bug"。
...
> **注意**：`datamap-query`、`datastudio-mcp` 依赖 `chrome-auth` 共享库，uvx 方式需要先 `pip install -e chrome-auth`。`datamap-query` 的写入功能还需配置 `DATAMAP_OPEN_API_TOKEN` 环境变量（见凭证获取章节），不配置不影响查询。
...
| **chrome-auth 认证** | 无需配置 | `scheduler-query`、`datamap-query`、`datastudio-mcp` 自动从 Chrome Cookie 获取认证，只需在 Chrome 中登录 [DataSuite](https://datasuite.shopee.io)。Spark 诊断工具还需登录 [Keyhole](https://keyhole.data-infra.shopee.io) |
