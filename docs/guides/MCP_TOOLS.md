...
**presto-query 补充（v3.6.3）**：`query_presto` 现使用 **`idc`** 参数切换集群，支持传 **`sg`** / **`us`**，默认 **`sg`**；返回结果会显示本次命中的 IDC，并统一附带 **实际执行的 SQL**（成功 / 失败 / 超时 / 请求异常），避免和业务 region 概念混淆。目录内另有 **`presto_query_tool.py`**，导出 **`PrestoQueryTool`** 类，便于在非 Cursor MCP 的 Python / 平台 Agent 中直连同一 DataSuite Personal SQL API（凭证仍由调用方传入，勿入库）。说明见 `mcp-tools/presto-query/README.md`。
