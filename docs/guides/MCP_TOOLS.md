...
- **自动 SSO 刷新**：检测到 Cookie 过期时，自动通过 AppleScript 控制 Chrome 打开 DataSuite 页面触发 SSO 静默续期（macOS），无需手动操作。若 Chrome 使用 `--remote-debugging-port=9222` 启动，还会优先使用 CDP 后台标签页刷新（更安静）
