...
├─────────────────────────────────┤
│ ● ACP  已连接 / 状态文案  [重连] │  ← ACP 连接状态（与左侧 rail 圆点同步；断开时显示「重连」）
├─────────────────────────────────┤
│                                 │
...
版本号、齿轮菜单等仍在 SeaTalk 左侧 **Agent rail**（星形图标旁）。存在可拉取的远程更新时，版本按钮会以红色胶囊样式显示向上箭头、「NEW」与目标版本（或仅提交落后数），点击打开更新浮层；更新完成后会短暂显示绿色「已同步」样式。面板内 ACP 条与 rail 上的连接圆点、状态文案同步更新。
...
- **连接状态（圆点）、版本号、齿轮菜单**在 SeaTalk **最左侧竖条**（`.sidebar-sidebar` 内 `#spx-agent-rail`），位于 **侧栏偏下**（圆点在上，版本号与齿轮在底部区域），在 SeaTalk 自带 **设置/帮助图标上方**，与 Cursor 星形按钮同一侧栏。**圆点可点击打开对话面板**（与 ✦ 等价）；断线重连请用 **齿轮 → 重连 ACP** 或对话面板内 **ACP 状态条** 的「重连」。齿轮菜单支持 Enter/Space 打开、Escape 关闭（输入框聚焦时不抢 Escape）。
...
1. 左侧 rail 圆点是否为绿色已连接；对话面板打开时看 **ACP** 行状态
