...
| MCP 状态   | 查看 MCP server / tool schema 状态，手动运行安全 smoke tests       |
| 检查更新     | 手动检查是否有新版本                                   |

### MCP 状态面板

SeaTalk Agent 设置菜单提供 **MCP 状态** 面板，用于排查 MCP 是否加载成功。它和 **诊断信息** / **查看后端日志** 共用 Agent 对话面板内的 overlay；若当前未打开面板，脚本会先打开 Agent 面板再显示状态。

- **重新检测**：重新读取 MCP 配置并执行 `tools/list`，不影响当前对话。
- **运行 Smoke**：手动执行内置安全 smoke tests，只覆盖只读、低成本工具。
- **重载 MCP 会话**：重建当前 Agent session，让 Codex/Cursor 重新加载 MCP schema。

第一版不会单独热重启某一个 MCP server；需要进程级重启时继续使用 **重启 Agent**。
...
- **MCP 状态面板**：左侧 Agent rail 齿轮新增 MCP 状态入口，展示 MCP server / tool schema 状态，支持重新检测、安全 smoke tests 与重载 MCP 会话；面板和诊断信息共用 Agent sidebar overlay，不再遮挡 SeaTalk 主界面。
