...
Node.js 后端 (seatalk-agent/src/main.ts)
  ...
  └── Updater       追踪 gitlab remote，自动版本检查与更新
...
Agent 会在后台自动检查新版本（追踪 **gitlab** remote 的 release 分支）：
...
> **注意**：自动更新功能要求本地配置了 `gitlab` remote（指向 `https://git.garena.com/tianyi.liang/spx-helper.git`）。如果你只有 `origin`（GitHub），需要添加：`git remote add gitlab https://git.garena.com/tianyi.liang/spx-helper.git`
...
*⏱ 12.3s | 2450↓ 680↑*
```
尾部显示回复时长和 token 消耗（输入↓ 输出↑）。无 token 信息时 fallback 为字数。
...
### v3.4.1
- **Remote 回复优化**：尾部显示回复时长和 token 消耗（输入↓ 输出↑），替代原来的时间+字数
