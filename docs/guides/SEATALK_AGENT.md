...
AI 可以帮你发送 SeaTalk 消息或添加好友，但需要你的确认。

**确认流程**：当 AI 调用发送消息或添加联系人时，消息流中会出现一张确认卡片，显示目标和内容预览。你可以：

- **确认发送 / 确认添加** — 执行操作
- **取消** — 拒绝操作

**免确认模式**：输入框上方有「免确认」开关（与 Model 选择同行）。开启后所有发送操作自动执行，不再弹确认卡片。适合需要批量发送的场景，用完记得关闭。

**安全机制**：
- MCP 工具无法直接发送消息，必须经过 Agent 的确认流程
- AI 只有在你**明确要求**时才会调用发送功能
